...
class Board:
    ...
    def _is_a_winning_play(self, realRow, column, piece):
        ...
        row = realRow - 1
        verticalCounter = 1
        while row >= 0 and verticalCounter < 4 and self.matrix[row][column] == piece:
            row -= 1
            verticalCounter += 1
        ...
        col = column - 1
        matrixRow = self.matrix[realRow]
        leftCounter = 1
        while col >= 0 and leftCounter < 4 and matrixRow[col] == piece:
            col -= 1
            leftCounter += 1
        if leftCounter < 4:
            col = column + 1
            horizontalCounter = leftCounter
            while col < Co.COLUMN_NUMBER and horizontalCounter < 4 and matrixRow[col] == piece:
                col += 1
                horizontalCounter += 1
        ...
        if botLeftCounter < 4:
            ...
            while row < Co.ROW_NUMBER and col < Co.COLUMN_NUMBER and diagOneCounter < 4 and self.matrix[row][col] == piece:
                col += 1
                row += 1
                diagOneCounter += 1
            if diagOneCounter > 3:
